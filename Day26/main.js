...
const sortCountry = (data,order) => {
   ...
   sortedCountry = order==="alphabetical"? sortedCountry : sortedCountry.reverse()
   lastEl.innerHTML = "" 
    sortedCountry.forEach(item => {
     const newEl  =document.createElement("div")
     newEl.textContent = item
     newEl.style.backgroundImage  =`url("./images/map_image.jpg")
     `
...
const searchCountryBy = (by,inputValue) => {
    ...
    if(by==="starting word"){
      ...
      countryDisplayed = countryNames.filter(country => country.toLocaleLowerCase().startsWith(inputValue.toLowerCase()))
    }
    else{
      console.log("selected search with any word")

      countryDisplayed = countryNames.filter(country => country.toLocaleLowerCase().includes(inputValue.toLowerCase()))
    }
...
buttonEl.forEach((btn,btnIndex) => {
    btn.addEventListener("click", e => {
      ...
      if(buttonText !== searchBy && btnIndex!==2 ){
        searchBy = buttonText
        buttonEl.forEach(item => { item.classList.contains("active")? item.classList.remove('active') : "" })
